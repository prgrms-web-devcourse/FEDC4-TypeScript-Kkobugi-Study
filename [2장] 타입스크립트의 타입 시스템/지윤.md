...
// 기존 Pick -> K는 T 타입과 무관하고 범위가 넓다.
type Pick<T, K> = { [k in K]: T[k] }; // 'K' 타입은 'string | number | symbol' 타입에 할당 할 수 없다.

// Pick -> K는 T의 키의 부분 집합(=keyof T)
type Pick<T, K extends keyof T> = { [k in K]: T[k] };
/*1. keyof 연산자를 통해 T 타입의 오브젝트 key들의 리터럴 유니온 타입을 만든다. -> ('userId' | 'pageTitle' | 'recentFiles' | 'pageContents')
 2. 아이템 7에서, 타입을 extends 하는 것은 해당 타입의 부분 집합을 의미
 3. 이 부분 집합을 K로 넘겨줌
 4. 매핑된 타입(keyof T)을 이용하여 T 타입의 키로 접근하여 타입의 속성에 대한 타입을 가져옴
*/
```
